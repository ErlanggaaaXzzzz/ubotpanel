...
const os = require("os");
const premiumDb = require("./lib/premium.js");
const ubotManager = require("./lib/userbotManager.js");
...
const getMenuText = (ctx) => {  
  ...
  const ownTag = `[${config.ownerName}](tg://user?id=${config.ownerId})`;  
  return `  
ようこそ | Welcome  
...
˖᯽ ݁˖ Speed: ${speed}ms  
˖᯽ ݁˖ Runtime: ${runtime(process.uptime())}  
˖᯽ ݁˖ Botmode: ${global.selfMode ? "Self" : "Public"}  
˖᯽ ݁˖ Developer: ${ownTag}  
˖᯽ ݁˖ Prefix: [${config.prefix}]  
˖᯽ ݁˖ Libray: Telegraf  
...
˖᯽ ݁˖ ${prefix}deladmin  
˖᯽ ݁˖ ${prefix}subdo  
˖᯽ ݁˖ ${prefix}installpanel  
˖᯽ ݁˖ ${prefix}startwings  
˖᯽ ݁˖ ${prefix}uninstallpanel  
`;  
  
const getOwnerMenu = () => `  
˖᯽ ݁˖ ${prefix}backup  
˖᯽ ݁˖ ${prefix}restart  
˖᯽ ݁˖ ${prefix}self / ${prefix}public  
˖᯽ ݁˖ ${prefix}eval  
˖᯽ ݁˖ ${prefix}cleardb  
`;  
          ...
          return ctx.answerInlineQuery([
            {
              type: "photo",
              id: "menu-1",
              photo_url: config.menuImage || "https://files.catbox.moe/bgtoiz.jpg",
              thumb_url: config.menuImage || "https://files.catbox.moe/y7yeip.jpg",
...
`;
        ...
        case "nsfw": {
          ...
          try {
            const cacheData = getCacheData(text);
            if (!cacheData) {
              // FIX: Menghapus variabel gaib 'q.id' agar tidak membuat bot crash
            ...
            return ctx.answerInlineQuery([
              {
                ...
                caption: cacheData.caption || "🖼 TikTok Slide Photo",
                parse_mode: "HTML",
                reply_markup: {
                  inline_keyboard: keyboard
                }
...
Pilih domain server:`,
  ...
  // ================= CHAT COMMANDS (bukan inline) =================
  // /start, .addprem, .delprem, .listprem, .addubot, .delubot, .restartubot
  // Dipisah dari inline_query karena flow OTP butuh percakapan 2 arah.

  const isPremiumUser = (userId) => isOwner({ from: { id: userId } }) || premiumDb.isPremium(userId);

  bot.start(async (ctx) => {
    const uid = ctx.from.id;
    const premiumBadge = isPremiumUser(uid) ? "✅ Premium" : "❌ Bukan Premium";
    let text = `👋 Selamat datang di *${config.botName}*!\n\nStatus akun kamu: ${premiumBadge}\n`;

    if (isPremiumUser(uid)) {
      text += `\n*Fitur Userbot (Premium)*\n`;
      text += `˖᯽ ݁˖ ${prefix}addubot <nomor>\n`;
      text += `˖᯽ ݁˖ ${prefix}delubot <nomor>\n`;
      text += `˖᯽ ݁˖ ${prefix}restartubot <nomor>\n`;
    } else {
      text += `\nFitur userbot hanya untuk member premium. Hubungi owner untuk berlangganan.\n`;
    }

    if (isOwner(ctx)) {
      text += `\n*Owner Tools*\n`;
      text += `˖᯽ ݁˖ ${prefix}addprem <userId> <durasi>\n`;
      text += `˖᯽ ݁˖ ${prefix}delprem <userId>\n`;
      text += `˖᯽ ݁˖ ${prefix}listprem\n`;
    }
  ...
  bot.on("text", async (ctx, next) => {
    try {
      ...
      // --- Tangkap balasan OTP / password 2FA saat proses login userbot berlangsung ---
      if (ubotManager.hasPendingLogin(uid) && !body.startsWith(prefix) && !body.startsWith("/")) {
        const awaiting = ubotManager.getAwaiting(uid);
        if (awaiting === "code") {
          ubotManager.submitCode(uid, body);
          return ctx.reply("⏳ Kode diterima, memverifikasi...");
        }
        if (awaiting === "password") {
          ubotManager.submitPassword(uid, body);
          return ctx.reply("⏳ Password diterima, memverifikasi...");
        }
        // sedang di antara langkah (misal Telegram lagi diproses) -> abaikan pesan ini
        return;
      }

      if (!body.startsWith(prefix)) return next();
      ...
      if (cmd === "addprem") {
        ...
        const [targetId, durasi] = args;
        if (!targetId || !durasi) {
          return ctx.reply(`Format: ${prefix}addprem <userId> <durasi>\nContoh durasi: 7d, 30d, 1y, 12h`);
        }
        const res = premiumDb.addPrem(targetId, durasi);
        if (!res.ok) return ctx.reply("❌ Durasi tidak valid. Contoh: 7d, 30d, 1y, 12h");
        return ctx.reply(
          `✅ User ${targetId} berhasil ditambahkan sebagai premium.\nBerlaku sampai: ${new Date(res.expireAt).toLocaleString("id-ID")}`
        );
      }
      ...
      if (cmd === "delprem") {
        if (!isOwner(ctx)) return ctx.reply("❌ Khusus owner.");
        const [targetId] = args;
        if (!targetId) return ctx.reply(`Format: ${prefix}delprem <userId>`);
        const res = premiumDb.delPrem(targetId);
        if (!res.ok) return ctx.reply("❌ User tersebut tidak ada di daftar premium.");
        return ctx.reply(`✅ Premium user ${targetId} telah dihapus.`);
      }

      // ---------- .listprem ----------
      if (cmd === "listprem") {
        if (!isOwner(ctx)) return ctx.reply("❌ Khusus owner.");
        const list = premiumDb.listPremium();
        if (!list.length) return ctx.reply("Belum ada user premium aktif.");
        const text = list
          .map((p, i) => `${i + 1}. ${p.userId} — expire: ${new Date(p.expireAt).toLocaleString("id-ID")}`)
          .join("\n");
        return ctx.reply(`*Daftar Premium Aktif*\n\n${text}`, { parse_mode: "Markdown" });
      }

      // ---------- .addubot ----------
      if (cmd === "addubot") {
        if (!isPremiumUser(uid)) return ctx.reply("❌ Fitur ini khusus member premium.");
        const [rawPhone] = args;
        if (!rawPhone) return ctx.reply(`Format: ${prefix}addubot <nomor_telepon>\nContoh: ${prefix}addubot +6281234567890`);
        if (ubotManager.hasPendingLogin(uid)) {
          return ctx.reply("⏳ Kamu masih punya proses addubot yang berjalan. Selesaikan dulu atau tunggu timeout.");
        }

        await ctx.reply(
          "⚠️ *Peringatan Keamanan*\n\n" +
          "Menambahkan userbot berarti kamu login memakai akun Telegram pribadimu di server ini. " +
          "Session hasil login disimpan di server dan berpotensi diakses oleh pengelola layanan. " +
          "Lanjutkan hanya jika kamu memahami dan menerima risiko ini.\n\n" +
          "Mengirim kode OTP ke nomor tersebut sekarang...",
          { parse_mode: "Markdown" }
        );

        const attempt = await ubotManager.startAddUbot(uid, rawPhone, {
          onNeedCode: () => ctx.reply("📩 Kode OTP telah dikirim ke akun Telegram tersebut. Balas pesan ini dengan kode OTP-nya."),
          onNeedPassword: () => ctx.reply("🔒 Akun ini memakai verifikasi 2 langkah. Balas dengan password 2FA-nya."),
        });

        if (!attempt.ok) {
          if (attempt.reason === "rate_limited") {
            const minutes = Math.ceil(attempt.retryAfterMs / 60000);
            return ctx.reply(`❌ Terlalu banyak percobaan addubot. Coba lagi dalam ${minutes} menit.`);
          }
          const reasons = {
            sudah_ada_proses_login: "Masih ada proses login lain yang berjalan untukmu.",
            nomor_sudah_terdaftar: "Nomor ini sudah terdaftar sebagai userbot kamu.",
          };
          return ctx.reply(`❌ ${reasons[attempt.reason] || "Gagal memulai proses login."}`);
        }

        // Login berjalan di CHILD PROCESS terpisah — kalau proses itu crash/OOM,
        // bot utama ini tidak ikut terganggu. Tunggu hasilnya di background.
        attempt.startPromise.then(() => {
          const failure = attempt.getFailure();
          if (failure) {
            return ctx.reply(`❌ Gagal login userbot: ${failure}\nSilakan ulangi dari awal dengan ${prefix}addubot.`);
          }
          return ctx.reply(`✅ Userbot untuk nomor ${rawPhone} berhasil terhubung dan siap dipakai.`);
        });
      ...
      if (cmd === "delubot") {
        if (!isPremiumUser(uid)) return ctx.reply("❌ Fitur ini khusus member premium.");
        const [rawPhone] = args;
        if (!rawPhone) return ctx.reply(`Format: ${prefix}delubot <nomor_telepon>`);
        const res = await ubotManager.delUbot(uid, rawPhone);
        if (!res.ok) return ctx.reply("❌ Userbot dengan nomor tersebut tidak ditemukan.");
        return ctx.reply(`✅ Userbot ${rawPhone} berhasil dihapus.`);
      }
      ...
      if (cmd === "restartubot") {
        if (!isPremiumUser(uid)) return ctx.reply("❌ Fitur ini khusus member premium.");
        const [rawPhone] = args;
        if (!rawPhone) return ctx.reply(`Format: ${prefix}restartubot <nomor_telepon>`);
        await ctx.reply("⏳ Merestart userbot...");
        const res = await ubotManager.restartUbot(uid, rawPhone);
        if (!res.ok) {
          const reasons = {
            tidak_ditemukan: "Userbot dengan nomor tersebut tidak ditemukan.",
            session_hilang: "File sesi userbot ini hilang, silakan addubot ulang.",
            gagal_connect: `Gagal konek ulang: ${res.detail || ""}`,
          };
          return ctx.reply(`❌ ${reasons[res.reason] || "Gagal restart userbot."}`);
        }
        return ctx.reply(`✅ Userbot ${rawPhone} berhasil direstart.`);
      }

      // ---------- .listubot ----------
      if (cmd === "listubot") {
        if (!isPremiumUser(uid)) return ctx.reply("❌ Fitur ini khusus member premium.");
        const list = ubotManager.listUserbots(uid);
        if (!list.length) return ctx.reply("Kamu belum punya userbot aktif.");
        const text = list.map((u, i) => `${i + 1}. ${u.phone} — ${u.status}`).join("\n");
        return ctx.reply(`*Userbot kamu*\n\n${text}`, { parse_mode: "Markdown" });
      }
      ...
      if (cmd === "cancelubot") {
        if (!isPremiumUser(uid)) return ctx.reply("❌ Fitur ini khusus member premium.");
        if (!ubotManager.hasPendingLogin(uid)) {
          return ctx.reply("Tidak ada proses addubot yang sedang berjalan.");
        }
        ubotManager.abortAddUbot(uid);
        return ctx.reply(`✅ Proses addubot dibatalkan. Kamu bisa mulai ulang dengan ${prefix}addubot.`);
      }

      return next();
    } catch (err) {
      console.log("❌ Text command error:", err);
    }
  });
        ...
        switch (menuType) {
          case "main":
            return ctx.editMessageMedia({
              type: "photo",
              media: config.menuImage || "https://files.catbox.moe/bgtoiz.jpg",
              caption: getMenuText(ctx),
              parse_mode: "Markdown"
            }, {
              reply_markup: {
